...
// Renders a data-entry form generated from a sheet's columns. On submit it
// inserts a new row (with an automatic timestamp) that shows up in the table.
export default function FormEntry({ sheet, cols, onSubmitted }) {
  ...
  function field(c) {
    ...
    if (c.type === 'number' || c.type === 'currency' || c.type === 'percent') {
      return <input type="number" value={v || ''} onChange={e => set(c.key, e.target.value)}
        placeholder={c.type === 'currency' ? '₹ amount' : c.type === 'percent' ? '%' : 'number'} />
    }
    return <input type="text" value={v || ''} onChange={e => set(c.key, e.target.value)} />
  }
